function [pano, L, S, S_disp, Lreg, Sreg, Sreg_disp] = PRPCA_noiseless(Yreg,Y,varargin)
%
...
[mask, height, width,lambdaS, nIters, cleanLS] ...
...
if isRGB
    ...
    S_disp = pano2video_RGB(Sreg_disp,mask,height,width,size(Y));
    ...
    % Generate forground/background video
    Lreg = reshape(Lhat,[height, width, size(Y,4)]);
    Sreg = reshape(Shat,[height, width, size(Y,4)]);
    M = logical(reshape(mask,[height, width, size(Y,4)]));
    ...
    S_disp = pano2video(Sreg_disp,mask,height,width,size(Y));
    % Generate panorama
    [uhat, ~, ~] = svds(reshape(Lreg,[],size(Y,4)),1);
...
% Parsed RPCA inputs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [mask, height, width, lambdaS, nIters, cleanLS] = parseRPCAInputs(Yreg,opts)
if ~exist('opts','var')
    opts = struct();
end
