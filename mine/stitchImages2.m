function [frames] = stitchImages2(It)
%
...
%
    ...
    for i = Ns:-1:1
        empty = nan(h,w,Nc);
        [frames{i},~,~,~] = overlayImage(empty,It(i),x,y);
    end
...
%--------------------------------------------------------------------------
function [Is, Ia, coord, indices] = overlayImage(Is,It,x,y)
    % Overlay image
    Nc = size(Is,3);
    [If,Ia, coord, indices] = fillImage(It,x,y);
    mask = ~any(isnan(If),3);
    for j = 1:Nc
        Isj = Is(:,:,j);
        Ifj = If(:,:,j);
...
%--------------------------------------------------------------------------
function [If,Ia, coord, indices]= fillImage(It,x,y)
    % Parse inputs
    Nc = size(It.image,3);
    w = numel(x);
    h = numel(y);
    ...
    indices = [yIdx1 yIdx2 xIdx1 xIdx2];
